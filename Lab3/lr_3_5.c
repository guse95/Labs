...
void HandlingError(const int code) {
    switch (code) {
        ...
        case SURNAME_TOO_LARGE: {
            printf("Surname is too large.\n");
            break;
        }
        ...
        case INVALID_NAME: {
            printf("Name or surname is invalid.\n");
            break;
        }
        case INVALID_STUDENT_DATA: {
            printf("Information about student is invalid.\n");
            break;
        }
        case TO_STR_FAIL: {
            printf("Failed to transform to string.\n");
            break;
        }
...
int AddElemToStudent(int pos, struct Student* Students, int student_Ind, char curElem[100]) {
    ...
    if (pos >= 4 && pos < 4 + 5) {
        Students[student_Ind].marks[pos - 4] = curElem[0];
    }
    return SUCCESS;
}
...
struct SelectedStudents find(int number_of_students, struct Student* Students, char el[100], char fl[8]) {
...
int main(int argc, char* argv[]) {
    ...
    if (strcmp(findCondition, "id") == 0) {
        ...
        scanf("%99s", surname);
        find_res = find(number_of_students, Students, surname, "surname");
    }
